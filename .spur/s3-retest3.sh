...
LOG="/Volumes/Projects/spur/.spur/s3-retest3.log"
cd "$WT" || { echo "FATAL cd" | tee -a "$LOG"; exit 99; }
: > "$LOG"
echo "## retest3 tip $(git rev-parse --short HEAD) start $(date -u +%H:%M:%S) (streamed)" | tee -a "$LOG"
step() {
  local name="$1"; shift
  echo "" | tee -a "$LOG"
...
rc=$?
echo "" | tee -a "$LOG"
echo "@@@@@@@@ RETEST3 DONE overall_rc=$rc $(date -u +%H:%M:%S) @@@@@@@@" | tee -a "$LOG"
exit $rc
