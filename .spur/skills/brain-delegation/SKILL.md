...
### Retrieving full worker output via `artifact`

When a delegation returns a non-null `artifact` field, the worker's full
stdout exceeded `SPUR_SUMMARY_MAX_BYTES` and was persisted as a git blob
on the orchestrator's side. The `summary` field is still truncated per
the context-budget invariant; the artifact is the source of truth for
long prose (research briefs, long diagnostics, design write-ups).

**Retrieve with:**

```bash
git cat-file -p <artifact.object_ref>
# or, stable across ref deletion:
git cat-file -p <artifact.blob_sha>
```

**When to fetch:**
- You are about to relay long research prose to the user.
- You hit a `Failed` status with `artifact.kind == "diagnostic"` and
  you need the full error context (compiler output, stack trace).

**When NOT to fetch:**
- The summary already contains what the user asked for. Do not
  re-inject full artifact text into your own context — that defeats
  the context-budget guarantee. If relaying prose, output artifact
  content directly in your response to the user without keeping it
  in working context.

**If `artifact` is absent AND `summary` contains `"[... N chars omitted ...]"`:**
Orchestrator attempted and failed to persist. Summary tail will contain
`[orchestrator: artifact persistence failed: ...]`. Retrieval is not
possible; inform the user and either re-delegate or proceed with the
truncated summary.
