...
async fn startup_sweep_resumes_after_prior_partial_child_quarantine() {
    if !br_available() {
        eprintln!(
            "skipping startup_sweep_resumes_after_prior_partial_child_quarantine: `br` not on PATH"
        );
        return;
    }
...
async fn startup_sweep_honors_plan_pending_grace_boundary() {
    if !br_available() {
        eprintln!("skipping startup_sweep_honors_plan_pending_grace_boundary: `br` not on PATH");
        return;
    }
...
async fn startup_sweep_quarantines_all_plan_children_with_comments() {
    if !br_available() {
        eprintln!(
            "skipping startup_sweep_quarantines_all_plan_children_with_comments: `br` not on PATH"
        );
        return;
    }
...
async fn startup_sweep_skips_pending_epic_without_plan_id_and_emits_event() {
    if !br_available() {
        eprintln!(
            "skipping startup_sweep_skips_pending_epic_without_plan_id_and_emits_event: `br` not on PATH"
        );
        return;
    }
