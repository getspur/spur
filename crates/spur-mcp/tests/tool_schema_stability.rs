...
use spur_mcp::tool_schemas::{
    DelegateAsyncInput, DelegateParallelInput, DelegateParallelTaskInput, DelegateToWorkerInput,
};
...
#[test]
fn test_delegate_async_schema_stability() {
    let schema = schema_for!(DelegateAsyncInput);
    let json = serde_json::to_value(&schema).unwrap();
    assert_eq!(json["title"], "DelegateAsyncInput");
    assert!(json["properties"].get("agent").is_some());
    assert!(json["properties"].get("task").is_some());
    assert!(json["properties"].get("delegation_plan").is_some());
    assert_eq!(json["additionalProperties"], json!(false));
