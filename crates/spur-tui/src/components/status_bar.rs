...
pub(crate) fn hint_for_session_detail(
    ...
    if stream_in_flight && !esc_consumed_by_composer {
        " [Enter]send [Esc]stop [j/k]scroll [Alt-m]plan [Alt-d]panel [Alt-e]workers [Ctrl-r]history [?]help"
    } else {
        " [Enter]send [Esc]back [j/k]scroll [Alt-m]plan [Alt-d]panel [Alt-e]workers [Ctrl-r]history [?]help"
    }
}
