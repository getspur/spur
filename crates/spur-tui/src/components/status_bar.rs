...
pub(crate) fn hint_for_session_detail(stream_in_flight: bool) -> &'static str {
    if stream_in_flight {
        " [Enter]send [Esc]stop [j/k]scroll [Alt-m]plan [Alt-w]workers [Ctrl-r]history [?]help"
    } else {
        " [Enter]send [Esc]back [j/k]scroll [Alt-m]plan [Alt-w]workers [Ctrl-r]history [?]help"
    }
}
