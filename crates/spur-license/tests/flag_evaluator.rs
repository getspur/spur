...
use spur_license::{FlagKey, InstallId, Tier};
...
fn kill_switch_disabled_flag_returns_false() {
    ...
    assert!(!evaluator.evaluate(FlagKey::KILL_ADVANCED_PLANNER, &spec, Tier::Community));
}
...
fn tier_filter_excludes_wrong_tier() {
    ...
    assert!(!evaluator.evaluate(FlagKey::KILL_ADVANCED_PLANNER, &spec, Tier::Community));
    assert!(evaluator.evaluate(FlagKey::KILL_ADVANCED_PLANNER, &spec, Tier::Pro));
}
...
fn rollout_is_deterministic() {
    ...
    let key = FlagKey::KILL_ADVANCED_PLANNER;
