...
/// Origin loop for a plan generated by a loop generation.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct PlanLoopOriginEvent {
    pub loop_id: String,
    pub generation: u32,
}
...
pub struct PlanSummaryEvent {
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub loop_origin: Option<PlanLoopOriginEvent>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
...
/// Loop row carried by `LoopsLoaded` for TUI list rendering.
///
/// Producer-side caps: `LoopsLoaded.loops` is capped at <= 200 rows, and
/// `goal_preview` is truncated before broadcast.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct LoopSummaryEvent {
    pub loop_id: String,
    pub issue_id: String,
    pub title: String,
    /// One of `l1`, `l2`, or `l3` when the loop declares an autonomy level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub last_cost_micros: Option<u64>,
...
/// One recent loop run shown in loop detail.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct LoopRunRecordEvent {
    pub generation: u32,
    pub outcome: String,
    pub cost_micros: u64,
    /// One of `l1`, `l2`, or `l3` when the run recorded an autonomy level.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub autonomy: Option<String>,
}

/// Full loop detail carried by `LoopDetailLoaded`.
///
/// Producer-side caps: `recent_runs` is capped at <= 20 records, and
/// `goal_preview` is truncated before broadcast.
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq, Eq)]
pub struct LoopDetailEvent {
    pub loop_id: String,
    pub issue_id: String,
    pub title: String,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    ...
    pub consecutive_failures: u32,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub budget_micros_per_generation: Option<u64>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_generations_per_day: Option<u32>,
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub max_tasks: Option<u32>,
    #[serde(default, skip_serializing_if = "Vec::is_empty")]
    pub recent_runs: Vec<LoopRunRecordEvent>,
}
...
pub enum SpurEventBody {
    ...
    /// Emitted when the TUI requests loop summaries.
    ///
    /// Producer-side cap: `loops` is capped at <= 200 rows. Producers also
    /// truncate each row's `goal_preview` before broadcast.
    LoopsLoaded {
        loops: Vec<LoopSummaryEvent>,
        #[serde(default, skip_serializing_if = "Vec::is_empty")]
        warnings: Vec<String>,
    },
    /// Emitted when the TUI requests full loop detail.
    ///
    /// Producer-side caps: `detail.recent_runs` is capped at <= 20 records,
    /// and `detail.goal_preview` is truncated before broadcast.
    LoopDetailLoaded {
        detail: LoopDetailEvent,
    },
    /// Feedback for a failed loop operation initiated from TUI.
    LoopCommandError {
        operation: String,
        #[serde(default, skip_serializing_if = "Option::is_none")]
