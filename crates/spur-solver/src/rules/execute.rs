...
use crate::types::{ModelValue, SolveConstraintsRequest, SolveConstraintsResponse, SolveStatus};
...
#[must_use]
pub fn finish(prepared: PreparedRuleSolve, solver: SolveConstraintsResponse) -> SolveRulesResponse {
...
pub const fn outcome_for(mode: RuleSolveMode, status: SolveStatus) -> RuleOutcome {
    match (mode, status) {
        (RuleSolveMode::Verify, SolveStatus::Sat) => RuleOutcome::Fail,
        (RuleSolveMode::Verify, SolveStatus::Unsat) => RuleOutcome::Pass,
