...
async fn wait_for_read_aggregate_comment(
    ...
    issue_id: &str,
    timeout: Duration,
) -> Option<spur_core::plan::audit_sentinel::AuditSentinelKind> {
    ...
    while tokio::time::Instant::now() < deadline {
        if let Ok(comments) = adv.list_comments(issue_id).await {
            for comment in comments {
                if let Some(Ok(kind)) =
                    spur_core::plan::audit_sentinel::parse_comment(&comment.body)
                {
                    if matches!(
                        kind,
                        spur_core::plan::audit_sentinel::AuditSentinelKind::ReadAggregate { .. }
                    ) {
                        return Some(kind);
                    }
                }
...
async fn flush_delegation_drains_buffer_and_emits_summary_once() {
    ...
    if let spur_core::plan::audit_sentinel::AuditSentinelKind::ReadAggregate {
        delegation_id,
        entries,
    } = sentinel
    {
