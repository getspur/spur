...
async fn new_runtime() -> Runtime {
    ...
    let reconciler = Reconciler::new(
        ...
        Some(ReconcilerDispatchCtx {
            ...
            event_sink: None,
            materializer: Arc::new(spur_core::outcome_materializer::OutcomeMaterializer::new(
                Arc::new(spur_blob_store::MemoryOutcomeStore::new()),
            )),
...
async fn validation_rejects_reuse_without_worker_branch() {
    let plan = Arc::new(tokio::sync::Mutex::new(spur_core::plan::PlanState {
        plan_id: "p1".into(),
    ...
    let result = spur_core::plan::handle_review_task(
        plan,
        "p1",
        "T1",
        "request_changes",
