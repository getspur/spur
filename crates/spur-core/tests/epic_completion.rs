...
use spur_core::plan::outcomes::{DispatchOutcome, OutcomeStore, SkipReason};
...
fn epic_completion_test_lock() -> std::sync::MutexGuard<'static, ()> {
    static LOCK: OnceLock<std::sync::Mutex<()>> = OnceLock::new();
    LOCK.get_or_init(|| std::sync::Mutex::new(()))
        .lock()
        .expect("epic completion test lock poisoned")
}
...
) -> spur_core::server::DetachedContinuationCtx {
    spur_core::server::DetachedContinuationCtx {
...
async fn mock_issue(
    pm: &spur_core::plan::test_util::MockPm,
    title: &str,
    body: &str,
    issue_type: &str,
    labels: Vec<String>,
...
async fn mock_pm_reconciler_terminal_failure_fires_escalated_task_continuation() {
    ...
    assert!(pm
        ...
        .labels
        .iter()
        .any(|label| label == spur_core::plan::mutation_executor::SIGNAL_ESCALATED_LABEL));
    task_tracker.close();
    tokio::time::timeout(COMPLETION_TASK_TIMEOUT, task_tracker.wait())
        .await
        .expect("mock completion tasks should finish");
