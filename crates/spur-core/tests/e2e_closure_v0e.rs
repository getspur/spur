...
use spur_core::plan::audit_sentinel::{AuditSentinelKind, EpicCompletionOutcome};
...
fn collect_sentinels(list_json: &str) -> Vec<AuditSentinelKind> {
    ...
    items
        ...
        .filter_map(spur_core::plan::audit_sentinel::parse_comment)
...
async fn seed_all_approved_epic(
    ...
    for (task_id, delegation_id, worker_branch) in [
        ...
        adv.add_comment(
            task_id,
            &spur_core::plan::audit_sentinel::encode_comment(&AuditSentinelKind::Completion {
                delegation_id: delegation_id.to_string(),
                completion_state: spur_core::plan::audit_sentinel::CompletionState::AwaitingReview,
                superseded: false,
        ...
        adv.add_comment(
            task_id,
            &spur_core::plan::audit_sentinel::encode_comment(&AuditSentinelKind::Approval {
                delegation_id: delegation_id.to_string(),
            }),
        )
        .await
...
impl ReconcilerAutomation for RecordingAutomation {
    async fn merge_plan(&self, plan_id: &str) -> anyhow::Result<spur_core::plan::PlanMergeState> {
        self.actions.lock().await.push(format!("merge:{plan_id}"));
        Ok(spur_core::plan::PlanMergeState::Succeeded {
...
async fn t_v0e_1_no_persisted_direct_dispatch() {
    ...
    server_exec.set_workers(vec![spur_core::WorkerInfo {
        name: "codex".into(),
        ..Default::default()
    }]);
