...
use spur_core::plan::mutation::{MutationBatch, PlanMutationOp, TaskDraft};
...
fn mutation_phase_2e_lock() -> MutexGuard<'static, ()> {
    static LOCK: OnceLock<Mutex<()>> = OnceLock::new();
    LOCK.get_or_init(|| Mutex::new(()))
        .lock()
        .expect("mutation_phase_2e test lock poisoned")
}
...
async fn mutation_op_insert_task_before_creates_dep_edge_and_resets_target() {
    let _lock = mutation_phase_2e_lock();
