...
//!
//! Guards INV-1 from the T1 contract-truthfulness spec: the set of core brain
//! tool names must not drift silently.
...
#[test]
fn delegation_tools_are_owned_by_core_catalog() {
    let actual: Vec<String> = tools_list().iter().map(|t| t.name.clone()).collect();
    for tool in [
        "delegate_to_worker",
        "delegate_parallel",
