...
use spur_core::outcome_materializer::OutcomeMaterializer;
use spur_core::plan::outcomes::OutcomeStore;
use spur_core::plan::reconciler::{
    PreviewStrategy, Reconciler, ReconcilerConfig, ReconcilerDispatchCtx,
};
use spur_core::plan::{projector, PlanTaskEntry};
...
async fn run_completion_collector_body(
    ...
    let deferred =
        match spur_core::plan::test_support::persist_worker_completion_and_notify_with_task_id(
            pm.as_ref(),
            &target.issue_id,
            feature_gate.as_ref(),
            PLAN_ID,
    ...
    spur_core::plan::test_support::prune_projected_terminal_task_outcomes_for_test(
        &outcomes,
        PLAN_ID,
        &projected.tasks,
    )
    ...
    spur_core::plan::snapshot::emit_plan_snapshot(Some(event_sink.as_ref()), &projected);
    tracing::info!(
        target: "spur.reconciler.completion_collector",
        plan_id = %PLAN_ID,
        task_id = %target.task_id,
...
fn verify_frozen_copy(repo: &Path) -> Result<()> {
    ...
    let plan_issue_count: i64 = conn.query_row(
        ...
        params![spur_core::plan::labels::plan_id(PLAN_ID)],
        |row| row.get(0),
    )?;
    anyhow::ensure!(
        plan_issue_count >= 12,
    ...
    let epic_count: i64 = conn.query_row(
        ...
        params![EPIC_ISSUE_ID, spur_core::plan::labels::plan_id(PLAN_ID)],
        |row| row.get(0),
    )?;
    anyhow::ensure!(
        epic_count == 1,
...
fn verify_project_shape(projected: &spur_core::plan::PlanState) -> Result<()> {
...
fn completion_targets_from_projected(
    projected: &spur_core::plan::PlanState,
    run: usize,
