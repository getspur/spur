...
async fn tick_once_does_not_dispatch_partial_plan_after_child_create_failure() {
    ...
    let err = spur_core::build_epic_subgraph(
        pm.as_ref(),
...
async fn persisted_overlay_conflict_emits_plan_task_blocked_on_setup_conflict_continuation() {
    ...
    assert!(matches!(
        &task.status,
        spur_core::plan::PlanTaskStatus::BlockedOnSetupConflict { dep_task_id, files }
...
async fn tick_once_persists_failed_completion_when_respond_to_drops() {
    ...
    assert!(matches!(
        task.status,
        spur_core::plan::PlanTaskStatus::Ready
    ));
...
async fn worker_success_after_orphan_clear_is_superseded() {
    ...
    let _deferred = spur_core::plan::test_support::persist_worker_completion_and_notify(
        pm.as_ref(),
        &task_id,
        common::server_builder::pro_feature_gate().as_ref(),
        "plan-race",
    ...
    let audits = spur_core::plan::projector::collect_sorted_audits(
...
async fn resolve_dispatch_orphan_emits_breadcrumb_and_clears_label() {
    ...
    let cleared = spur_core::server::resolve_dispatch_orphan(
        Arc::clone(&pm),
        common::server_builder::pro_feature_gate(),
        &task_id,
    )
    ...
    assert!(audits.iter().any(|audit| matches!(
        ...
        AuditSentinelKind::DispatchOrphanCleared {
            ...
            && reason == spur_core::plan::test_support::ORPHAN_CLEAR_REASON_RESTART
    )));
}
...
async fn execute_epic_persists_execution_scope_labels_on_epic_and_tasks() {
    ...
    server.set_workers(vec![spur_core::WorkerInfo {
        name: "codex".into(),
        ..Default::default()
    }]);
    pm.update_issue(
