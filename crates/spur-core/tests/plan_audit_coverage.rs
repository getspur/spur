...
use spur_core::plan::audit_sentinel::{
    self, AuditSentinelKind, CompletionAuditFields, CompletionState,
};
use spur_core::plan::{PlanState, PlanTask, PlanTaskEntry, PlanTaskStatus};
use tempfile::TempDir;
use tokio::sync::Mutex;

mod common;

fn plan_audit_coverage_lock() -> MutexGuard<'static, ()> {
    static LOCK: OnceLock<StdMutex<()>> = OnceLock::new();
    LOCK.get_or_init(|| StdMutex::new(()))
        .lock()
        .expect("plan_audit_coverage test lock poisoned")
}
...
async fn add_labels_individually(pm: &spur_pm::PmService, issue_id: &str, labels: &[String]) {
    ...
    spur_core::plan::persist_dispatch_intent(
        &pm,
        &issue_id,
    ...
    assert!(
        issue
            .labels
            .contains(&spur_core::plan::labels::delegation_id("del-A")),
...
async fn plan_audit_coverage_all_four_sentinels() {
    let _lock = plan_audit_coverage_lock();
    let dir = TempDir::new().expect("tempdir");
    run_br(dir.path(), &["init"]).expect("br init failed");
    ...
    spur_core::emit_plan_submit_audit(
        adv,
        "audit-plan-1",
        &subgraph,
        spur_core::PlanSubmitAuditContext {
    ...
    let pm_arc: Arc<dyn spur_core::plan::PmLike> = Arc::new(pm);

    let issue_id_opt = Some(task_issue_id.clone());
    spur_core::plan::emit_dispatch_audit(
        Some(pm_arc.as_ref()),
        &issue_id_opt,
        common::server_builder::pro_feature_gate().as_ref(),
        "audit-plan-1",
    ...
    let plan_state = PlanState {
        ...
        merge_state: spur_core::plan::PlanMergeState::NotStarted,
        epic_id: Some(epic_issue_id.clone()),
    };
    let plan_arc_state = Arc::new(Mutex::new(plan_state));

    spur_core::plan::handle_review_task(
        plan_arc_state,
        "audit-plan-1",
        "t1",
        "approve",
...
async fn epic_completion_audit_round_trips_through_collect_sentinels() {
    ...
    let adv = pm.advanced().expect("beads backend must have advanced()");
    spur_core::plan::emit_epic_completion_audit(
        adv,
        &epic_id,
        "audit-plan-epic",
        spur_core::plan::audit_sentinel::EpicCompletionOutcome::AllApproved,
    )
    .await
    ...
    assert!(epic_sentinels.iter().any(|audit| matches!(
        ...
        AuditSentinelKind::EpicCompletion {
            outcome: spur_core::plan::audit_sentinel::EpicCompletionOutcome::AllApproved,
            plan_id,
...
async fn dispatch_intent_includes_lease_label() {
    ...
    let issue_id = pm
        .create_issue(spur_pm::IssueCreate {
            ...
            labels: vec![
                spur_core::plan::labels::READY_FOR_REVIEW.to_string(),
        ...
        .await
        .expect("create issue");
    let stale_lease = spur_core::plan::labels::lease_expires_at(1_700_000_000);
    ...
    assert!(issue.labels.iter().any(|label| {
        spur_core::plan::labels::parse_lease_expires_at(label).is_some() && label != &stale_lease
    }));
    assert!(!issue.labels.contains(&stale_lease));
    assert!(!issue
        .labels
        .contains(&spur_core::plan::labels::READY_FOR_REVIEW.to_string()));
...
async fn completion_success_writes_ready_for_review_and_completion_audit() {
    ...
    spur_core::plan::persist_completion_result(
        &pm,
        &issue_id,
        common::server_builder::pro_feature_gate().as_ref(),
        "plan-1",
...
async fn completion_failed_closes_issue_and_emits_completion_audit() {
    ...
    add_labels_individually(
        ...
        &[
            spur_core::plan::labels::READY_FOR_REVIEW.to_string(),
            "ready-for-review".to_string(),
            spur_core::plan::labels::delegation_id("del-fail"),
    ...
    assert!(!issue
        .labels
        .contains(&spur_core::plan::labels::delegation_id("del-fail")));
...
async fn completion_cancelled_closes_issue_and_emits_completion_audit() {
    ...
    add_labels_individually(
        ...
        &[
            spur_core::plan::labels::READY_FOR_REVIEW.to_string(),
            "ready-for-review".to_string(),
            spur_core::plan::labels::delegation_id("del-cancel"),
    ...
    assert!(!issue
        .labels
        .contains(&spur_core::plan::labels::delegation_id("del-cancel")));
    assert!(!issue
        .labels
...
async fn reject_closes_issue_and_adds_review_rejected_label() {
    ...
    let pm_arc: Arc<dyn spur_core::plan::PmLike> = pm.clone();
    ...
    assert!(issue
        .labels
        .contains(&spur_core::plan::labels::REVIEW_REJECTED.to_string()));
    assert!(!issue
        .labels
        .contains(&spur_core::plan::labels::READY_FOR_REVIEW.to_string()));
    assert!(!issue.labels.contains(&"ready-for-review".to_string()));
