...
use spur_core::plan::audit_sentinel::{AuditSentinelKind, CompletionState};
...
use spur_core::plan::{PlanState, PlanTaskStatus, PmLike};
...
impl ReconcilerDispatch for SimDispatch {
    ...
    fn continuation_ctx(&self) -> &Arc<spur_core::server::DetachedContinuationCtx> {
...
impl Harness {
    ...
    fn from_existing_pm_with_config(
        ...
        let continuation_ctx = Arc::new(spur_core::server::DetachedContinuationCtx {
...
impl SimPm {
    ...
    async fn project(&self) -> PlanState {
        spur_core::plan::projector::project_plan_from_beads(
            self,
            PLAN_ID,
...
impl SimState {
    ...
    fn add_audit(&mut self, issue_id: &str, kind: AuditSentinelKind) {
        self.add_comment(
            issue_id,
            spur_core::plan::audit_sentinel::encode_comment(&kind),
        );
    }
...
impl BeadsAdvanced for SimPm {
    ...
    async fn add_comment(&self, issue_id: &str, body: &str) -> anyhow::Result<String> {
        ...
        if state.delay_projection_after_completion
            && matches!(
                spur_core::plan::audit_sentinel::parse_comment(body),
        ...
        if let Some(AuditSentinelKind::Dispatch { .. }) =
            spur_core::plan::audit_sentinel::parse_comment(body).and_then(Result::ok)
        {
            let task_id = state
                .task_issue_by_id
                .iter()
