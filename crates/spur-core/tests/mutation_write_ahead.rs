...
use spur_core::plan::labels::signal_processed_label;
use spur_core::plan::mutation::{DepRewirePolicy, MutationBatch, PlanMutationOp, TaskDraft};
use spur_core::plan::mutation_executor::apply_mutation;
...
async fn compensate_mutation_orphans_emits_violation_breadcrumb() {
    ...
    run_br(
        ...
        &[
            ...
            "add",
            &child,
            &spur_core::plan::labels::mutation_id_label(&mutation_id),
        ],
    )
    ...
    spur_core::server::compensate_mutation_orphans(
        Arc::clone(&pm),
        common::server_builder::pro_feature_gate(),
        &parent,
    )
