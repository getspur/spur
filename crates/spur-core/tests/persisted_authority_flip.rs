...
use spur_core::plan::proposers::{ScopeDriftSplitProposer, TrivialScorer};
use spur_core::plan::reconciler::{Reconciler, ReconcilerConfig, ReconcilerDispatchCtx};
use spur_core::plan::signal_watcher::SignalWatcher;
use spur_core::plan::signals::{self, WorkerSignal};
use spur_core::plan::{PlanMergeState, PlanState, PlanTask, PlanTaskEntry, PlanTaskStatus};
use spur_core::server::{
...
async fn t_v0c_6_watcher_uses_projected_plan_state_not_stub_state() {
    ...
    assert!(
        issue
            .labels
            .contains(&spur_core::plan::mutation_executor::SIGNAL_ESCALATED_LABEL.to_string()),
...
async fn t_v0c_7_cache_miss_rehydrates_persisted_plan_from_beads() {
    ...
    run_br(dir.path(), &["init"]).expect("br init");
    let pm = beads_pm(dir.path()).await;
    let _subgraph = spur_core::build_epic_subgraph(
        pm.as_ref(),
        common::server_builder::pro_feature_gate().as_ref(),
        "plan-7",
        "Plan Seven",
