...
async fn review_approve_releases_plan_lock_before_beads_io() {
    ...
    let state = spur_core::plan::PlanState {
        plan_id: "p1".into(),
        tasks: vec![spur_core::plan::PlanTaskEntry {
    ...
    let plan_arc: Arc<Mutex<spur_core::plan::PlanState>> = Arc::new(Mutex::new(state));
    ...
    let (sleepy_pm, entered_rx) =
        spur_core::plan::test_support::make_sleepy_pm_with_signal(Duration::from_secs(1));
