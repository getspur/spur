...
use spur_core::plan::labels::{self};
use spur_core::plan::mutation::{MutationBatch, PlanMutationOp};
use spur_core::plan::mutation_executor::{apply_mutation, submit_plan_mutation};
use tempfile::TempDir;
use uuid::Uuid;

mod common;

fn mutation_phase_2c_lock() -> MutexGuard<'static, ()> {
    static LOCK: OnceLock<Mutex<()>> = OnceLock::new();
    LOCK.get_or_init(|| Mutex::new(()))
        .lock()
        .expect("mutation_phase_2c test lock poisoned")
}
...
async fn mutation_op_retry_task_resets_to_pending_preserves_history() {
    let _lock = mutation_phase_2c_lock();
...
fn legacy_taskspec_audit_round_trips_without_extended_fields() {
    ...
    match parsed {
        AuditSentinelKind::TaskSpec {
            ...
            assert!(planned_write_files.is_none());
