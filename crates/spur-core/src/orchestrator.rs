...
use session::{abort_mcp_handle, cleanup_mcp_on_err, retire_brain_session, ReconnectDisposition};
...
impl Orchestrator {
    ...
    pub fn register_notebook_socket(&self, brain_session_id: spur_acp::BrainSessionId) {
        ...
        self.notebook_sockets
            .write()
            .expect("notebook socket registry lock poisoned")
            .insert(brain_session_id, nonce.clone());
        self.emit(SpurEvent::now(SpurEventBody::NotebookSocketReady {
            session,
            socket_nonce: nonce,
        }));
    }
