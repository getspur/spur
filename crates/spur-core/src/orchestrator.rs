...
pub mod test_support {
    ...
    pub const MCP_SHUTDOWN_TIMEOUT_MS: u64 =
        super::session::MCP_SHUTDOWN_TIMEOUT.as_millis() as u64;
    ...
    pub async fn call_shutdown_mcp_server<S: RetirableMcpServer + ?Sized>(
        ...
        let mut adapted: Option<Arc<dyn super::session::RetirableMcpServer>> =
            mcp_server.map(|s| {
                Arc::new(RetirableMcpServerAdapter(s))
                    as Arc<dyn super::session::RetirableMcpServer>
            });
        let mut guard_slot: Option<AbortOnDropHandle<()>> = mcp_guard;
        super::session::shutdown_mcp_server(funnel, session, &mut adapted, Some(&mut guard_slot))
            .await;
    }
