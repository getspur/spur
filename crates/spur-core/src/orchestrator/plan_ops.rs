use spur_acp::{
    ...
    PlanSummaryCountsEvent, PlanSummaryEvent, SessionId,
};
...
pub(super) fn loop_origin_from_labels(labels: &[String]) -> Option<PlanLoopOriginEvent> {
    let loop_id = labels
        .iter()
...
pub(super) async fn load_plan_summaries<P: crate::plan::PmLike + ?Sized>(
    pm: &P,
    current_brain_session: Option<&SessionId>,
    ...
    for epic in epics {
        ...
        candidates.push(PlanSummaryCandidate {
            summary: PlanSummaryEvent {
                ...
                loop_origin: loop_origin_from_labels(&epic.labels),
                counts,
                updated_at,
                created_at,
            },
...
pub(super) async fn annotate_plan_summary_canonical_epics<P: crate::plan::PmLike + ?Sized>(
    pm: &P,
...
mod plan_summary_warning_tests {
    use super::*;
    use crate::plan::{labels, test_util::MockPm, PmLike};
    ...
    #[tokio::test]
    async fn load_plan_summaries_populates_loop_origin_from_epic_labels() {
        let pm = MockPm::new();
        let loop_epic_id = pm
            .create_issue(spur_pm::IssueCreate {
                title: "Loop-authored plan".into(),
                description: Some("Generated by a loop generation".into()),
                issue_type: Some("epic".into()),
                labels: vec![
                    labels::plan_id("plan-loop"),
                    labels::loop_id_label("loopabc"),
                    labels::loop_generation_label(7),
                ],
                ..Default::default()
            })
            .await
            .expect("create loop plan epic");
        pm.create_issue(spur_pm::IssueCreate {
            title: "Loop id without generation".into(),
            description: Some("Missing one provenance label".into()),
            issue_type: Some("epic".into()),
            labels: vec![
                labels::plan_id("plan-missing-generation"),
                labels::loop_id_label("loopabc"),
            ],
            ..Default::default()
        })
        .await
        .expect("create partial loop plan epic");

        let load = load_plan_summaries(&pm, None)
            .await
            .expect("load plan summaries");

        let loop_plan = load
            .plans
            .iter()
            .find(|plan| plan.plan_id == "plan-loop")
            .expect("loop plan summary");
        assert_eq!(loop_plan.epic_id, loop_epic_id);
        let origin = loop_plan.loop_origin.as_ref().expect("loop origin");
        assert_eq!(origin.loop_id, "loopabc");
        assert_eq!(origin.generation, 7);

        let partial = load
            .plans
            .iter()
            .find(|plan| plan.plan_id == "plan-missing-generation")
            .expect("partial loop plan summary");
        assert_eq!(partial.loop_origin, None);
    }
}
