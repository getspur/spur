...
impl BrainScheduler {
    ...
    pub fn push_continuation(&mut self, c: BrainContinuation) {
        ...
        if self.active_session.as_ref() != Some(&c.brain_session) {
            tracing::debug!(
    ...
    pub fn note_session_swap(&mut self, new_active: Option<SessionId>, overflow: &OverflowBuf) {
...
mod tests {
    ...
    fn test_drained_batch_leaked_channel_full_emits_dropped() {
        let session = SessionId::new();
        let (mut scheduler, sink) = mk_scheduler_with_capacity(Some(session.clone()), 1);
    ...
    fn test_note_session_swap_drains_pending_and_overflow() {
        ...
        scheduler.note_session_swap(Some(new_session.clone()), &overflow);

        assert_eq!(scheduler.pending_continuation_len(), 0);
        assert!(overflow.try_lock().unwrap().is_empty());
        assert_eq!(scheduler.active_session, Some(new_session));
