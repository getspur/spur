...
async fn concurrent_writes_no_corruption() {
    tokio::time::timeout(Duration::from_secs(20), async {
        ...
        // 4 simulated instances x 25 writes each = 100 issues. The
    ...
    .await
    .expect("concurrent writes should complete before the deadline");
}
...
async fn concurrent_first_open_serializes_via_migration_lock() {
    tokio::time::timeout(Duration::from_secs(10), async {
        ...
        let h1 =
            tokio::spawn(
                async move { BeadsCrateAdapter::open(&p1, AdapterConfig::default()).await },
            );
        let h2 =
            tokio::spawn(
                async move { BeadsCrateAdapter::open(&p2, AdapterConfig::default()).await },
            );
    ...
    .await
    .expect("concurrent first open should complete before the deadline");
}
...
async fn snapshot_conflict_detected() {
    ...
    .await
    .expect("snapshot conflict test should complete before the deadline");
}
