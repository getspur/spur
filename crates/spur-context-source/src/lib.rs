...
mod tests {
    ...
    #[test]
    fn dns_check_resolves_localhost_to_loopback_and_rejects() {
        // Unlike the tests above (which use `parsed_for_host` with an
        // IP-literal hostname and hit the fast `hostname.parse::<IpAddr>()`
        // path), "localhost" is not an IP literal, so this exercises the
        // real `to_socket_addrs()` resolution loop in
        // `resolve_and_check_dns` for the first time. It resolves purely via
        // /etc/hosts (no network I/O), so it is deterministic and fast.
        let err = resolve_and_check_dns(&parsed_for_host("localhost")).unwrap_err();
