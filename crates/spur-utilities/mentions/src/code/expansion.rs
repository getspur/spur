//! Validated expansion of a code mention payload into prompt text
//! (2026-08-27 mentions spec, shared `code` module; relocated verbatim from
//! `spur-tui` by the sud-m4 decoupling phase).
//!
//! [`expand`] validates the payload's spec against the worktree on every
//! call (no cached expansion): a passing file or symbol range expands to a
//! [`ExpandedMention::Body`] with the capped signature + context header; a
//! failing one expands to a [`ExpandedMention::Warning`] naming the failure
//! reason and the degradation (`file_mention` replacement or `dropped`).
//! The byte caps ([`PER_PROMPT_CAP_BYTES`], [`CONTEXT_HEADER_CAP_BYTES`]) are
//! part of the public contract — the TUI submit router budgets prompt
//! mention bodies against them.
...
fn signature_line(line: &str) -> String {
    match line.find('{') {
        Some(index) => line[..=index].trim().to_owned(),
...
mod tests {
    ...
    fn first_signature_line_extracts_function_signature() {
        ...
        assert_eq!(
            ...
            Some("pub fn run(args: Args) -> Result<()>".to_owned())
        );
    }
    ...
    fn first_signature_line_truncates_over_cap() {
        ...
        let expected = {
            let mut line = source.lines().next().expect("signature").trim().to_owned();
            line.pop();
    ...
    fn first_signature_line_captures_where_clause() {
        ...
        assert_eq!(
            first_signature_line(source, [start, end], "run"),
            Some("pub fn run<T>( value: T, ) -> Result<T> where T: Clone,".to_owned())
        );
    }
