use jute::commands::venv::venv_list_impl;
use rmcp::{
...
const METHOD: &str = "notebook.venv_list";

pub fn tool() -> Tool {
    Tool::new(
        METHOD,
        "List Python virtual environments managed by the notebook app.",
        rmcp_object(json!({
            "type": "object",
            "properties": {},
...
pub async fn call(deps: &ServerDeps, arguments: Value) -> Result<CallToolResult, McpError> {
    ...
    let venvs = venv_list_impl(app)
        .await
        .map_err(|error| jute_error(METHOD, &error))?;
    Ok(CallToolResult::structured(json!(venvs)))
}
