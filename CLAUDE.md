...
- `scripts/spur-cargo e2e`: run the TUI e2e layer (remote by default like `run`; the VM builds `spur-cli`, runs `scripts/e2e/run-all.sh`, and syncs failure artifacts back to `scripts/e2e/.artifacts/`). Use `SPUR_E2E_ONLY=behavioral|visual` to select one suite, `SHELL_USE_RUNS=N` to repeat shell-use journeys, and `SPUR_REMOTE=0` only for local runs where `target/debug/spur` or `SPUR_BIN` already exists. Journey ownership lives in `scripts/e2e/JOURNEYS.md`; authoring rule: looks -> vhs tape, does -> shell-use journey.
...
- `scripts/spur-cargo zigbuild --release -p spur-cli`: **the standard macOS build path** — cross-compiles a Mach-O binary on the build VM (defaults to `aarch64-apple-darwin`; pass `--target universal2-apple-darwin` for a fat arm64+x86_64 artifact). zig compiles the C/C++ (DuckDB etc.), clang+ld64.lld links against Apple SDK stubs with the **system libc++** (required so natively-built DuckDB extension dylibs can exchange exceptions with the binary). Fetch artifacts with `scripts/cloud-build/fetch.sh --via-s3 target/<triple>/release/spur`. The VM toolchain self-restores from S3 at boot; provision a live VM or refresh the S3 bundle with `scripts/zigbuild-provision-vm.sh [--publish-bundle]` (run from a Mac with Xcode CLT). If a build stops on libproc's missing `osx_libproc_bindings.rs` (only after RUSTFLAGS changes), run the VM plant helper and rebuild. Details: `docs/superpowers/specs/2026-07-07-zigbuild-macos-cross-poc.md`.
- `scripts/spur-cargo xwin build --release -p spur-cli`: **the standard Windows build path** — cross-compiles a PE binary on the build VM (defaults to `--target x86_64-pc-windows-msvc`). cargo-xwin downloads the MSVC CRT + Windows SDK from Microsoft (cached under `/mnt/cargo/xwin`), clang-cl compiles the C/C++ (DuckDB etc.), lld-link links. Usage mirrors cargo's verbs (`xwin build`/`xwin check`). Fetch artifacts with `scripts/cloud-build/fetch.sh --via-s3 target/x86_64-pc-windows-msvc/release/spur.exe`. The toolchain self-provisions on a fresh VM at boot (apt + GitHub + Microsoft, no bundle); provision a live VM with `scripts/xwin-provision-vm.sh`. Details: `docs/superpowers/specs/2026-07-07-xwin-windows-cross-poc.md`.
- `cargo xtask dist [--platforms linux,macos,windows] [--out <dir>]`: **the release entry point for the full platform matrix** — builds `spur` for Linux (native aarch64 on the VM), macOS (universal2 fat binary via zigbuild), and Windows (x86_64 PE via xwin), all remotely, then fetches the artifacts into `dist/` as `spur-<version>-<triple>[.exe]` plus a `SHA256SUMS`. Works from any host since every build is remote.
