...
- **Stale pack data downgrades layer 1 to orientation only.** If `knowledge_context_pack` reports stale graph metadata (`response_file_oids_match: Some(false)`, an `indexed_head_oid` behind HEAD, or stale/staleness warnings), do not rely on its source bodies, line numbers, or impact counts. Keep only its identifiers/file hints, then re-ground with realtime `code_*` calls (`code_symbol_search`, `code_read_symbol`, `code_callers`, `code_callees`) before answering or editing.
...
- **Trusting stale graph data silently.** Every response includes `indexed_head_oid`, `worktree_dirty`, and `response_file_oids_match`.
  ...
  - If a `knowledge_context_pack` response is stale, treat it as a search hint, not grounding. Fall back to realtime `code_*` tools for the exact symbols/files before making claims, because those calls re-read the current worktree instead of trusting the stale pack payload.
