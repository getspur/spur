...
class TerminalTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.assertTrue(hasattr(probe, "TerminalHost"), "probe must implement advertised terminals")
        self.host = probe.TerminalHost(self.responses.put, Path(self.tmp.name), "strict")
        self.addCleanup(self.host.close)
        self.request_id = 0

    def send(self, method: str, **params: object) -> int:
        self.request_id += 1
        self.assertTrue(self.host.handle({"jsonrpc": "2.0", "id": self.request_id,
                                         "method": method, "params": {"sessionId": "s", **params}}))
    ...
    def test_nonzero_exit_preserves_output_and_allows_next_command(self) -> None:
        ...
        self.assertEqual(self.call("terminal/wait_for_exit", terminalId=terminal)["result"]["exitCode"], 7)
        ...
        self.assertEqual(self.call("terminal/release", terminalId=terminal)["result"], {})
        terminal = self.create("/bin/sh", ["-c", "printf recovered"])
        self.call("terminal/wait_for_exit", terminalId=terminal)
        self.assertEqual(self.call("terminal/output", terminalId=terminal)["result"]["output"], "recovered")
    ...
    def test_spawn_failure_is_rpc_error_then_host_remains_usable(self) -> None:
        ...
        self.assertEqual(self.call("terminal/wait_for_exit", terminalId=terminal)["result"]["exitCode"], 0)

    def test_grok_compat_is_explicit_and_preserves_quoted_script(self) -> None:
        script = "printf '%s' 'quoted; 世界'"
        packed = "/bin/bash -lc " + shlex.quote(script)
        self.assertIn("error", self.call("terminal/create", command=packed))
        self.host.mode = "grok"
        terminal = self.create(packed, [])
        self.call("terminal/wait_for_exit", terminalId=terminal)
        self.assertEqual(self.call("terminal/output", terminalId=terminal)["result"]["output"], "quoted; 世界")
    ...
    def test_stdin_eof_cwd_and_env_are_honored(self) -> None:
        terminal = self.create("/bin/sh", ["-c", 'cat; printf "%s:%s" "$PROBE_VALUE" "$PWD"'],
    ...
    def test_utf8_truncation_retains_tail_and_zero_limit(self) -> None:
        for limit, expected in [(4, "界x"), (0, "")]:
            with self.subTest(limit=limit):
                terminal = self.create(sys.executable, ["-c", "print('a世界x', end='')"], outputByteLimit=limit)
                self.call("terminal/wait_for_exit", terminalId=terminal)
    ...
    def test_wait_does_not_block_kill_or_other_callbacks(self) -> None:
        ...
        responses = {r["id"]: r for r in [self.responses.get(timeout=5), self.responses.get(timeout=5)]}
        self.assertEqual(responses[kill_id]["result"], {})
        self.assertIsNotNone(responses[wait_id]["result"]["signal"])
        self.assertIn("exitStatus", self.call("terminal/output", terminalId=terminal)["result"])
    ...
    def test_terminal_is_scoped_to_session(self) -> None:
        terminal = self.create("/bin/sh", ["-c", "exit 0"])
        self.assertIn("error", self.call("terminal/output", terminalId=terminal, sessionId="other"))
...
class ContinuationTests(unittest.TestCase):
    def test_probe_records_follow_up_after_terminal_failure(self) -> None:
        self.assertTrue(hasattr(probe, "TerminalHost"), "live callback support is missing")
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            fake = root / "fake.py"
            fake.write_text(FAKE_AGENT)
            args = probe.parse_cli(["--command", sys.executable, "--args", str(fake),
                                    "--cwd", tmp, "--out", str(root / "frames.jsonl"),
                                    "--report", str(root / "report.json"), "--no-try-set",
                                    "--prompt", "fail shell", "--follow-up-prompt", "continue",
                                    "--terminal-mode", "strict", "--preamble-timeout", "0", "--quiet"])
            with redirect_stdout(io.StringIO()):
                self.assertEqual(probe.run_probe(args), 0)
            report = json.loads((root / "report.json").read_text())
            self.assertEqual(len(report["prompt_results"]), 2)
            self.assertEqual(report["prompt_result"], report["prompt_results"][0]["rpc"])
            self.assertEqual(report["prompt_results"][0]["failed_tool_calls"], ["failed-shell"])
            self.assertEqual(report["prompt_results"][1]["assistant_text"], "FOLLOW_UP_OK")
            ...
            init = next(r["msg"] for r in frames if r["msg"].get("method") == "initialize")
