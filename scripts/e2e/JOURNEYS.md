...
| `problem-plan-loop-drive` | Problem: submit_plan auto-loop opaque. Plan browser + lineage brain↔worker navigation + detail tabs + activity; optional brain kick. | demo-live | real project | `Plan`; `Progress`; `[Agents]`; `stream`; `attempts`; `Activity` | `scripts/e2e/demos/tui-live/tapes/13-problem-plan-loop-drive.tape` |
