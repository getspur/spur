...
| `problem-plan-loop-drive` | Problem: submit_plan auto-loop opaque. Plan browser + lineage brain↔worker nav + optional live 1-task submit_plan seed (`SPUR_DEMO_ALLOW_PLAN_LOOP=1`). | demo-live | real project | `Plan`; `Progress`; `[Agents]`; `stream`; `EXEC`/`Running` (when seeded) | `scripts/e2e/demos/tui-live/tapes/13-problem-plan-loop-drive.tape` |
