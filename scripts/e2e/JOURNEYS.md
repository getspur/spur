...
| `product-e2e-flow` | Live long E2E: switch sessions → explore star skill+agent → gate accept/apply → `@worker` cascade (profile/model/effort); optional send. | demo-live | real project | `Lineage`; `Sessions`; `synced`; `Gate`; `applied`; `Mentions`; `agent=`; `model=`; `effort=` | `scripts/e2e/demos/tui-live/tapes/09-product-e2e-flow.tape` |
