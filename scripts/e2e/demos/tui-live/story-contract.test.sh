...
assert_not_matches <(awk '/^trigger_submit_plan_hitl_review_and_synthesize\(\) \{/ { in_hitl=1 } in_hitl { print } in_hitl && /^}/ { exit }' "$lib") '(?m)^[[:blank:]]+story_hard_proof "The (?:positioning result exposes|proof result exposes|retry exposes|readiness result exposes)' \
