...
assert_has "$lib" 'real brain + four workers + one retry' \
  'PH audit spend guard advertises all four initial workers'
assert_lacks "$lib" 'real brain + three workers + one retry' \
  'PH audit spend guard removes the obsolete three-worker cost'
...
assert_matches "$lib" '(?m)^plan_inspector_task_detail_text\(\) \{[[:blank:]]*\n^[[:blank:]]+"\$shell_use_bin" --session "\$session_name" text --full \| awk '\''[[:blank:]]*\n^[[:blank:]]+\{[[:blank:]]*\n^[[:blank:]]+if \(!in_detail\) \{[[:blank:]]*\n^[[:blank:]]+if \(index\(\$0, "Task detail"\) > 0\) \{[[:blank:]]*\n^[[:blank:]]+in_detail = 1[[:blank:]]*\n^[[:blank:]]+\}[[:blank:]]*\n^[[:blank:]]+next[[:blank:]]*\n^[[:blank:]]+\}[[:blank:]]*\n^[[:blank:]]+boundary = index\(\$0, "││"\)[[:blank:]]*\n^[[:blank:]]+if \(boundary == 0\) \{[[:blank:]]*\n^[[:blank:]]+exit[[:blank:]]*\n^[[:blank:]]+\}[[:blank:]]*\n^[[:blank:]]+right = substr\(\$0, boundary \+ length\("││"\)\)[[:blank:]]*\n^[[:blank:]]+print right[[:blank:]]*\n^[[:blank:]]+\}[[:blank:]]*\n^[[:blank:]]+'\''[[:blank:]]*\n^\}[[:blank:]]*$' \
  'PH audit extracts only the right-hand Task detail pane below its heading'
assert_matches "$lib" '(?m)^story_plan_inspector_task_status_hard_proof\(\) \{[[:blank:]]*\n^[[:blank:]]+local claim="\$1"[[:blank:]]*\n^[[:blank:]]+local task_id="\$2"[[:blank:]]*\n^[[:blank:]]+local status="\$3"[[:blank:]]*\n^[[:blank:]]+local dwell="\$\{4:-3\.0\}"[[:blank:]]*\n^[[:blank:]]+local timeout_s="\$\{5:-\$\{SPUR_DEMO_PLAN_LOOP_WAIT_S:-180\}\}"[[:blank:]]*\n^[[:blank:]]+local deadline=\$\(\(SECONDS \+ timeout_s\)\)[[:blank:]]*\n^[[:blank:]]+local detail=""[[:blank:]]*\n(?:^\n)*^[[:blank:]]+while \(\( SECONDS < deadline \)\); do[[:blank:]]*\n^[[:blank:]]+detail="\$\(plan_inspector_task_detail_text \|\| true\)"[[:blank:]]*\n^[[:blank:]]+if \[\[ "\$detail" == \*"task: \$\{task_id\}"\* && "\$detail" == \*"status \$\{status\}"\* \]\]; then[[:blank:]]*\n(?:^[[:blank:]]+.*\n|^\n)*?^[[:blank:]]+return 0[[:blank:]]*\n^[[:blank:]]+fi[[:blank:]]*\n(?:^[[:blank:]]+.*\n|^\n)*?^[[:blank:]]+done[[:blank:]]*\n(?:^[[:blank:]]+.*\n|^\n)*?^[[:blank:]]+printf [^\n]*fatal: Plan Inspector Task detail never exposed[^\n]*\n^[[:blank:]]+"\$shell_use_bin" --session "\$session_name" text --full >&2 \|\| true[[:blank:]]*\n^[[:blank:]]+return [1-9][0-9]*[[:blank:]]*\n^\}[[:blank:]]*$' \
  'PH audit hard-waits task id and status inside one bounded Task detail pane'
...
assert_not_matches <(awk '/^trigger_submit_plan_hitl_review_and_synthesize\(\) \{/ { in_hitl=1 } in_hitl { print } in_hitl && /^}/ { exit }' "$lib") '(?m)^[[:blank:]]+story_hard_proof[^\n]*"approved"' \
  'PH audit approval proofs never match the aggregate Plan Inspector header globally'
