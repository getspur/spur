...
assert_matches "$lib" '(?m)^workers_panel_text\(\) \{[[:blank:]]*\n^[[:blank:]]+"\$shell_use_bin" --session "\$session_name" text \| awk '\''[[:blank:]]*\n^[[:blank:]]+index\(\$0, "Workers \("\) > 0 \{ in_workers = 1 \}[[:blank:]]*\n^[[:blank:]]+in_workers \{ print \}[[:blank:]]*\n^[[:blank:]]+in_workers && index\(\$0, "Alt\+D collapse"\) > 0 \{ exit \}[[:blank:]]*\n^[[:blank:]]+'\''[[:blank:]]*\n^\}[[:blank:]]*$' \
  'PH audit extracts only the visible Workers panel from terminal text'
...
assert_matches "$lib" '(?m)^trigger_submit_plan_hitl_review_and_synthesize\(\) \{[[:blank:]]*\n(?:^[[:blank:]]+.*\n|^\n)*?^[[:blank:]]+story_hard_proof[^\n]*"THINK"[^\n]*\n(?:^[[:blank:]]+.*\n|^\n)*?^[[:blank:]]+press_key Alt\+d[[:blank:]]*\n^[[:blank:]]+story_workers_panel_hard_proof[^\n]*"Workers \(3\)"[^\n]*\n^[[:blank:]]+story_workers_panel_hard_proof[^\n]*"claude-code"[^\n]*\n^[[:blank:]]+story_workers_panel_hard_proof[^\n]*"gemini"[^\n]*\n^[[:blank:]]+story_workers_panel_hard_proof[^\n]*"codex"[^\n]*\n^[[:blank:]]+story_dwell[^\n]*\n^[[:blank:]]+press_key Alt\+d[[:blank:]]*$' \
  'PH audit worker beat binds all four anchors to the current Workers panel'
