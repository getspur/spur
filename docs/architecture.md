...
SPUR's primary deployment target is **local filesystems** (developer workstations, single-host CI runners). NFS, sshfs, SMB, and other network filesystems that lack POSIX advisory-lock support (`ENOTSUP`/`ENOLCK`) are **not** in the supported deployment matrix. Risks tagged R4-conditional below (Risk #4-R4a fs_unsafe skip, Risk #41 fs_unsafe multi-instance gap, Orphan Monopoly interaction) are therefore **not on the live risk surface** for the supported deployment. They are retained in the table for completeness and to document the consequence model if a network-FS deployment is ever attempted, but they do not contribute to the operational Systemic Health Score for the supported configuration.
...
| 2 | `broadcast::channel` drops events when subscribers are slow | High | **Partial** | R5 | 0.45 | `app.rs:2437` | Capacity is 4096, but TUI drain cap is **still 8** (not 64 as previously claimed). Bot has **no drain cap**. On `Lagged`, all subscribers log a warning and **permanently drop** events. No replay-from-NDJSON exists. Startup-replay path landed in bd-1vnk closes the cold-start half; live-Lagged recovery still open. |
...
| 4 | Worktree orphaning on unclean shutdown | ~~High~~ **Low–Medium** | **Partially addressed** | R1/R2/R4 | 0.15 | `worktree_authority.rs:99`, `orchestrator.rs:1106` | `WorktreeAuthority` deployed with `SessionLivenessProbe` + startup/periodic sweep. Original `cleanup_orphans` dead-code issue superseded. Residual: snapshot branches leak (R4b, no sweep), legacy pre-v2 namespaces skipped (R4c). **R4a (`fs_unsafe` skips all cleanup) is N/A for the supported local-FS deployment** — see Deployment Context above. Score lowered from 0.25 → 0.15 to reflect the local-only scope. See `docs/rca/2026-04-26-risk4-mcts-first-principles-evaluation.md` and `docs/rca/2026-04-27-full-architecture-mcts-first-principles-evaluation.md` §2.1/2.2. |
...
| 9 | Broadcast `Lagged` recovery not implemented | ~~Low~~ **High** | **Partial** | R3/R5 | 0.60 | `app.rs`, `event_sink.rs`, `notification_pump.rs` | Every subscriber logs `warn!` and continues. EventSink writes NDJSON but **code now reads it back at TUI startup (bd-1vnk)** for replay. MCTS: this is a backpressure amplifier — under load, lineage projection diverges from ground truth. Severity raised from Low per `docs/rca/2026-04-27-full-architecture-mcts-first-principles-evaluation.md` §2.3. Half-A (startup replay) landed in bd-1vnk; half-B (Lagged-during-live recovery) tracked separately. |
...
| 41 | **`fs_unsafe` multi-instance gap on NFS/sshfs** — When the repository lives on a network filesystem that lacks advisory-lock support (`ENOTSUP`/`ENOLCK`), SPUR attaches with `fs_unsafe=true` and proceeds without any cross-process exclusion. Two TUI windows on different hosts mounting the same NFS export can silently attach to the same session. | ~~Medium~~ **N/A (local deployment)** | **Open (out-of-scope)** | R4 | 0.00 | `session_lock.rs:33`, `orchestrator.rs` | The `DegradedNoLock` path is intentional (attach is better than failing), but there is no secondary coordination mechanism (e.g., beads issue label, TCP consensus) to backfill the missing filesystem guarantee. **Out of scope for the supported local-FS deployment** — see Deployment Context above. Score lowered to 0.00 for the live risk surface. If a network-FS deployment is later supported, severity reverts to High and remediation reopens. |
...
| ~~**Orphan Monopoly**~~ (out of scope) | #4-R4a (fs_unsafe skip) | #41 (no secondary consensus) | NFS-deployment-only interaction. Out of scope for the supported local-FS deployment — see Deployment Context. Retained for completeness if network-FS support is later added. | ~~0.80~~ **N/A** |
...
9. ~~**Secondary coordination for `fs_unsafe` (Risks #4-R4a, #41)**~~ — **Deferred (out of scope).** Local-FS-only deployment makes this unnecessary. Re-open if network-FS support enters the deployment matrix.
...
16. ~~**Session lock `fs_unsafe` backfill (Risk #41)**~~ — **Deferred (out of scope).** Same rationale as item 9: local-FS deployment does not surface this. Re-open if network-FS support is later added.
