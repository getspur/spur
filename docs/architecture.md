...
| 3 | Silent `_ => {}` catch-alls on `SpurEventBody` matches | High | **Open** | R3/R6 | 0.55 | `app.rs:959,1035` | Two catch-all arms remain. 37 of 74 variants fall through silently in brain-status tracking. First catch-all (`:1031`) is completely silent — no log. |
...
| 24 | **NDJSON rotation death spiral on disk full** — `rotate()` calls `flush()` then opens a new file. If `flush()` fails (disk full), `bytes_in_file` is never reset. Every subsequent event triggers another rotation attempt, fails, and is dropped. Permanent silent data loss until restart. | **High** | **Open** | R1/R2 | 0.75 | `event_sink.rs:100` | No `StorageFull` handling. The error path does not reset internal counters, so the sink enters a permanent failure mode. Compounds with Risk #29 (SQLite BUSY) under disk pressure. |
...
|---|---|---|---|---|
| **Disk Death Spiral** | #24 (NDJSON rotation fails) | #29 (SQLite BUSY) | Disk full → NDJSON dies silently; cost queries fail with `SQLITE_BUSY`; user sees neither cost nor events. | **0.85** |
...
1. **Eliminate silent `_ => {}` catch-alls (Risks #3, #37)** — Replace every catch-all arm in production event handlers with explicit `warn!` + metric. Use `#[deny(unreachable_patterns)]` or `#[non_exhaustive]` helpers that force match arms with measurable default behavior. Expected risk reduction: R3 drops 0.95 → 0.40; interaction multiplier drops 1.8× → 1.2×.
...
5. **Harden EventSink against disk full (Risk #24)** — On `flush()` failure, enter `StorageDegraded` mode: stop rotation attempts, emit a single `StorageFull` event, and write to a bounded in-memory ring buffer. Reset `bytes_in_file` before every rotation decision.
