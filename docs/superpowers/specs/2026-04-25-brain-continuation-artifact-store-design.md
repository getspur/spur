...
- **Reviewers:** codex (rounds 1–2, 5b, 6a, 7, 10a), kimi (rounds 3–4, 5a, 6b), gemini (rounds 5c, 7, 10b), L9 grounding pass (round 9 — see `docs/superpowers/reviews/2026-04-25-brain-continuation-l9-evaluation.md`)
...
| 10a | codex (round-10 verification) | 2026-04-25 | REJECT-WITH-MUST-FIX | Round 9's GC-M1 sidecar layout (`refs/spur/artifacts/<session>/<delegation>-<attempt>.meta`) collides with git's directory/file rule — the legacy ref at `refs/spur/artifacts/<session>` is a leaf, blocking any children. Verified with `git check-ref-format` + `git update-ref` repro. Other round-9 items held (P1-M1 flatten, P1-M2 ordering, N1 type-property clip, N2/N3 fallback flow). Plus 2 SHOULD-FIX (proptest char distribution; GitBlob UUID validation) + 1 NIT (golden fixture variants). |
| 10b | gemini (round-10 verification) | 2026-04-25 | REJECT-WITH-MUST-FIX | Confirmed codex's D/F conflict finding via independent path. Surfaced an additional MUST-FIX: the legacy `refs/spur/artifacts/<session-id>` ref is **shared per session** — every delegation overwrites the same ref. With OutcomeKey `(session, delegation, attempt)` granularity, blobs for earlier delegations are silently lost. Round 9 inherited this latent bug from current code; the sidecar-amendment exposed it. Plus SHOULD-FIX: `OutcomeMetadata` missing `sha256` field → forces full re-hash on every duplicate put. L9 grounding pass legitimacy spot-check confirmed authentic substantive folds (P1-M1, GC-M1, P2-S2 verified). INV-D8 enforced-by-clip + N3 fallback: solid hardening, not a no-progress loop. |
| 11 | self (round-10 fold) | 2026-04-25 | — | **MF1 + MF2**: rewrote §6.4 to introduce a NEW ref namespace `refs/spur/outcomes/<session-id>/<delegation>-<attempt>.{blob,meta}` for Plan-5 storage; legacy `refs/spur/artifacts/<session-id>` becomes read-only-during-Phase-1-transition. Per-`(session, delegation, attempt)` granularity restored. No D/F conflict (both new refs are leaves under the namespace). Backcompat adapter §6.5 returns the real new-namespace ref. **SF1**: added `sha256: String` to `OutcomeMetadata`; both backends read it directly for ContentMismatch detection (no re-hashing). **SF2**: §4 `arb_artifact_ref` generator explicitly includes null/control/backslash/high-bit chars (not just length variations). **SF3**: §6.4 `GitBlobOutcomeStore::put` validates UUID-shape on identifiers (parallel to FsOutcomeStore). **NIT**: §5.1 golden fixture covers BOTH unit variant (`Patch`) and data-carrying variant (`Other(String)`) to lock both top-level discriminator and `name` projection. §8.4 sweep now walks both new + legacy namespaces; legacy refs (no sidecar) treated as `created_at = epoch` and pruned on first sweep. |
