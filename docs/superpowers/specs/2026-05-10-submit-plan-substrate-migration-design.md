...
| INV-S5 multi-label atomicity gap: `persist_dispatched_base_oid_label`'s `update_issue` is not atomic across add+remove; process death between them can leave a stale label that recovery selects. | MED | Eliminate by adding an atomic multi-label primitive to beads-rust, OR by deduplicating dispatched-base-oid labels at recovery time (prefer most-recent by issue audit-comment ordering). Track for follow-up; not blocking PR9. |
