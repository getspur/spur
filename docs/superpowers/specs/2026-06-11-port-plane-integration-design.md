...
**Date:** 2026-06-11
**Status:** Draft for review (design approved in principle; supersedes nothing — builds on the
Jute-App container design)
**Related:**
...
Rules: **control messages carry refs, not payloads** (except a small `signal` class), and
**the data plane stays dumb**. Together these make the eventual `ipc://` bus a transport swap,
not a redesign.
...
| `signal` | **inline in the `PortPut` event**, ≤ 16 KiB | none (no file) | latest-wins per port | sliders, selections, cursors, heartbeats |

`signal` ports still get monotonic versions and appear in the manifest (value stored in the
manifest entry, not as a file), so `spur.get` works uniformly. The threshold (16 KiB initial) is
an invariant-guarded constant, not a convention.

```mermaid
flowchart LR
    subgraph SIGNAL["signal class - control-plane-resident"]
...
The engine task is the **single writer**; `seq` is strictly monotonic (invariant I1 — the same
discipline `SpurEvent.seq` already enforces in `spur-core`). v1 transport: in-process
`tokio::sync::broadcast` plus a bounded in-memory ring for late subscribers; persistence is an
open question (§12), not a v1 requirement.
...
flowchart TD
    ...
    PRV --> CACHE["Run skipping:<br/>same cell version + same input refs as last success<br/>-> RunFinished{skipped_fresh} without executing"]
...
| ID | Invariant | Guard |
|---|---|---|
| I1 | `PortEvent.seq` strictly monotonic; single writer (engine task) | constructor private to engine; test asserts ordering under concurrent intents |
| I2 | `port@version` content immutable once written | PortStore refuses overwrite of existing version; test |
| I3 | Control messages carry refs; inline payloads only `signal` class ≤ 16 KiB | intent-door check V4; constant in one place |
| I4 | `manifest.json` remains the kernel-facing data-plane index | no kernel/bootstrap changes required in v1; bootstrap compat tests unchanged |
...
**Build order within v1:** (a) event types + log + derived `dagStatusChanged` (I1/I5 tests
first, TDD); (b) `CascadeError` + `IntentRejected` surfacing (W4.2 lands here); (c) intent-door
unification + `emits` (W7.1); (d) `RunStarted.inputs`/`RunFinished.outputs` provenance + skip;
(e) declared schemas + `signal` class.

## 11. Non-goals (v1)

- Multi-client, multi-window, headless runner (v2 bus).
- Cross-notebook or cross-app ports.
- Streaming beyond the `signal` class (no incremental Arrow batches on a port version).
...
2. **Signal threshold** — 16 KiB initial; measure real widget payloads before freezing.
