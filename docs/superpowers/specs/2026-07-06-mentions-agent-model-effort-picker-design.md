...
- Each subsequent whitespace-delimited token is fuzzy-matched against the **current open slot's** candidate pool. A high-confidence match (or explicit accept from the dropdown) freezes that slot and advances to the next. `Slot::Model` candidates come from the cache's `.models` for that worker name (empty/"probing…" if cold); `Slot::Effort` from `.efforts`.
- Any slot can be left unfilled — ending the mention (space + non-`@` text, or submit) with slots still open leaves them `None`.
- A token that doesn't fuzzy-match anything in the current slot above threshold is **not consumed** — the mention finalizes with remaining slots at `None`, and the token becomes ordinary prompt text. This guarantees the picker never eats real user text on a near-miss.
...
| `crates/spur-core` | `Orchestrator::check_agents` (`orchestrator/connection.rs:54-77`) gains `session/new`+shutdown per agent, calling into the new `spur-acp` probe helper and persisting the result via the shared cache module. |
| `crates/spur-tui/src/mentions/` | `entry.rs` (`MentionEntry` new fields), `worker_source.rs`/`registry.rs` (completion state machine, `Slot` enum, agent/model/effort candidate sourcing from the shared cache), `hint.rs` (`prepend_worker_hint` URI parsing extension). |
