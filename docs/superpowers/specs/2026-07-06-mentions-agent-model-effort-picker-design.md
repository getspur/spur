...
- ~~**No visual/graphical picker widget.**~~ **Superseded (2026-07-07):** the original plan was blind continuous typing with no visible candidate list, matching today's `@`-trigger dropdown. Real usage showed this forces the user to already know a valid agent/model/effort value with zero discoverability. Reconsidered to reuse the *existing* list-based picker pattern already shipped for `/model`/`/effort` (`ConfigOptionQuerySource`) instead of inventing new chrome — see §6.5. The original blind-typing/comma-delimited mechanism (§6.2) is kept as a parallel power-user path, not replaced.
...
- **An empty token (`,,`, i.e. two commas with nothing between them) explicitly skips the current slot** without attempting a match, e.g. `@codex,,gpt-5,high` skips agent and goes straight to model=gpt-5, effort=high. Discovered necessary during implementation verification: when a worker has real `.spur/agents/*.md` profiles (so the Agent slot has live candidates, not auto-skipped), a user who wants to specify a model without an agent has no other way to move past the Agent slot — typing a model name directly would fail to match any agent candidate and, per the "not consumed" rule above, finalize the *entire* mention with everything from that point on treated as ordinary unconsumed text, silently dropping the model/effort intent. A single trailing comma with nothing typed after it yet does **not** count as a skip (still reads as "waiting" mid-type, matching pre-cascade behavior) — only a genuinely empty segment *between* two commas is a skip signal. This preserves the "never eat a near-miss" guarantee (auto-falling back across slots on any non-match was considered and rejected — it would risk misreading ordinary prose typed right after a worker mention as a model/effort guess).
...
### 6.5 List-based per-slot picker (added 2026-07-07, supersedes the §3 no-visual-widget non-goal)

Reuses the exact pattern already shipped for `/model`/`/effort` (`ConfigOptionQuerySource`: fuzzy-filtered selectable rows, `accept(row_idx)` returns the picked value) instead of inventing new UI. The data was already correctly scoped per-worker (`agent_slot_candidates` filtered by `AgentKind`, catalog lookup keyed by worker name) — what changed is *displaying* it instead of requiring blind typing.

- **`MentionRegistry::compose_worker_mention`** now returns `ComposedWorkerMention { entry, open_slot: Option<OpenWorkerSlot> }`. `OpenWorkerSlot` captures whichever slot (`WorkerSlotKind::{Agent,Model,Effort}`) the cascade hasn't resolved past yet, with its full (fuzzy-filtered, ranked) candidate pool, `splice_at` (byte offset into the query where a picked value gets spliced), and `is_final` (whether no later slot has real candidates, so picking here finishes the mention). Exposed via `MentionRegistry::take_worker_open_slot()`, read once per `query()` call — same "side-channel drain" pattern already used for `pending_agent_model_catalog_probe_requests`.
- **`MentionQuerySource::refresh`** renders `open_slot.candidates` as the row list instead of the single composed row when a slot is open; `accept(row_idx)` returns `RetrievalAccept::ReplaceTriggerToken` (splice `,<value>,` at `splice_at`, keep the popup open) for a non-final slot, or the existing `InsertAtom` path (via the new `finalize_worker_slot_pick` helper, which fills in whichever field the final slot resolves and rebuilds the atom exactly like blind-typing-through would) for the final slot.
- **New `QuerySource::accept_keeps_open(row_idx) -> bool`** trait method (defaulted `false`, zero-touch for every other source) and **new `PickerAction::AcceptKeepOpen`** variant let `input_completion.rs`'s `handle_picker_key` apply a non-final accept without closing the popup, then recompute the query substring and push it to the shell directly — bypassing the trigger detector's own `step()`, since every `IntentEvent` it accepts either closes the popup or doesn't apply to an out-of-band buffer splice.
- **Two bugs only surfaced by driving the real TUI live** (invisible to `MentionRegistry`-only unit tests, since both live in the UI-dispatch layer): (1) `apply_accept`'s `ReplaceTriggerToken` handling always re-anchors to the trigger detector's own `prefix_start` (correct for `/model`'s full-region replacement, wrong for a partial mid-buffer splice) — fixed by having the `AcceptKeepOpen` arm call `replace_trigger_token` directly with the accept's own `prefix_start`, not through `apply_accept`. (2) The spliced replacement was missing its leading comma (`splice_at` always lands right after the previous segment with *no* separating comma yet) — fixed by prefixing the replacement with `,`. Both are covered by `worker_cascade_picker_advances_through_agent_model_effort_and_commits_final_atom` in `input_completion.rs`, which drives the full dispatch → accept → re-render loop rather than calling `MentionRegistry::query` directly.
- The blind-typing comma/double-comma-skip mechanism (§6.2) is unaffected and still works standalone — both paths share the same `WorkerMentionSlot` state machine and tokenizer.
