        ...
        loop {
            {
                ...
                if let Some(conn) = free.pop() {
                    return Ok(ReaderGuard { pool: self, conn: Some(conn) });
                }
            ...
            let conn = SqliteStorage::open(&self.beads_dir)?;
            return Ok(ReaderGuard { pool: self, conn: Some(conn) });
        }
    }
}
...
mod tests {
    ...
    fn init_beads(dir: &std::path::Path) {
        ...
        let _ = SqliteStorage::open(dir).expect("open initializes schema");
    }
...
Expected: 2 pass. If beads_rust schema init requires extra setup, adjust `init_beads` accordingly (consult `beads_rust::storage::sqlite::SqliteStorage::open` docs).

- [ ] **Step 4: Commit**

```bash
git add crates/spur-pm/src/beads_crate/
git commit -m "spur-pm: add ReaderPool for WAL-concurrent reader connections"
```
