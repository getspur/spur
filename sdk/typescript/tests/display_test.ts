...
Deno.test("display.html exposes Symbol.for('Jupyter.display')", () => {
  ...
  assertExists(obj[DISPLAY_SYMBOL]);
});
...
Deno.test("DISPLAY_SYMBOL is Symbol.for('Jupyter.display')", () => {
  // Verify the runtime identity matches what Deno-Jupyter looks up.
  assertEquals(DISPLAY_SYMBOL, Symbol.for("Jupyter.display"));
});
